...
@export var first_person_offset := Vector3(0, 0.5, -0.5) 

@onready var target: RigidBody3D = get_parent()

@onready var body: Node3D = target.get_node('wagon_wheelless')
...
@onready var wheel: Sprite2D = $Handle/EmmaHandle

var first_person := false
var cam_distance := 5.0
var handle_pos := Vector2(576,282)
...
func _physics_process(delta: float) -> void:
	if ((Input.is_action_just_pressed("change_pov")) && (GlobalData.selected_car_scene == "res://Wagon/Wagon.tscn")):
	...
	if first_person:
		if Input.is_action_pressed("turn_left"):
			wheel.position = handle_pos + Vector2(-158,39)
			wheel.rotation = deg_to_rad(-35)
		elif Input.is_action_pressed("turn_right"):
			wheel.position = handle_pos + Vector2(158,39)
			wheel.rotation = deg_to_rad(35)
		else:
			wheel.position = handle_pos
			wheel.rotation = 0

	if first_person:
		global_transform.origin = target.global_transform.origin + target.global_transform.basis * first_person_offset
