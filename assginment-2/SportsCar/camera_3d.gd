...
@onready var body: Node3D = target.get_node('CAR Model(1)')
...
@onready var wheel: Sprite2D = $SteeringWheel/RyanWheel
...
func _physics_process(delta: float) -> void:
	if ((Input.is_action_just_pressed("change_pov")) && (GlobalData.selected_car_scene == "res://SportsCar/SportCar.tscn")):
