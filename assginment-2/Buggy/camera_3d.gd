...
@export var first_person_offset := Vector3(0, 0.3, -0.9) 
@export var wheel_rotation_speed := 5.0

@onready var target: RigidBody3D = get_parent()

@onready var body: Node3D = target.get_node('Buggy')
@onready var console: CanvasLayer = $Console
@onready var wheel: Sprite2D = $SteeringWheel/AidenWheel
...
func _physics_process(delta: float) -> void:
	if ((Input.is_action_just_pressed("change_pov")) && (GlobalData.selected_car_scene == "res://Buggy/Buggy.tscn")):
